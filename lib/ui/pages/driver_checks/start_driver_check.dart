...
class StartDriverCheckPage extends StatefulWidget {
  @override
  _StartDriverCheckState createState() => new _StartDriverCheckState();
}

class _StartDriverCheckState extends State<StartDriverCheckPage> {
  ...
  initialise() async {
    ...
    questions = await getDriverCheckQuestions(companyId, vehicleId);
  ...
  _getUser() async {
    ...
    driverCheck.driverName = name;
  }

  initialiseDriversCheck() {
    this.driverCheck = new CreateDriverChecks(
        vehicleId,
  ...
  sendDriversChecks() async {
    ...
    try {
      ...
      var response =
          await dio.post(BASE_URL + "/api/driverchecks/Create", data: body);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            body: isLoading
                ? loadingSplash(
                    context, "Getting driver check questions", "Please wait")
                : (isSending
                    ? loadingSplash(
                        context, "Sending driver check", "Please wait")
                    : ListView(children: [
