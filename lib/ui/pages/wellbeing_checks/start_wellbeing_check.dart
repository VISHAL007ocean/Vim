...
import 'package:vim_mobile/common/api_functions/wellbeing_checks/getWellbeingCheckQuestions.dart';
import 'package:vim_mobile/common/api_functions/wellbeing_checks/postWellbeingChecksImages.dart';
...
import 'package:vim_mobile/models/wellbeing_checks/CreateWellbeingCheck.dart';
...
class StartWellbeingCheckPage extends StatefulWidget {
  @override
  _StartWellbeingCheckState createState() => new _StartWellbeingCheckState();
}

class _StartWellbeingCheckState extends State<StartWellbeingCheckPage> {
  ...
  CreateWellbeingChecks wellbeingCheck = new CreateWellbeingChecks(0, 0, 0, "",
      "", "", 0, 0, new List<CreateWellbeingCheckAnswers>(), "", "");
  TextEditingController wellbeingNameController = TextEditingController();
  ...
  String backMessage =
      "Leaving this page will result in you losing all progress of your current wellbeing check.";
  ...
  @override
  void initState() {
    print("start wellbeing init");
    initialise().then((result) {
      setState(() {
        print("start wellbeing false");
        isLoading = false;
        isSending = false;
      });
    });
  ...
  onImagePressed(CreateWellbeingCheckAnswers answer) async {
  ...
  initialise() async {
    debugPrint("start wellbeing initilise");
    SharedPreferences preferences = await SharedPreferences.getInstance();
    _preferences = preferences;
    vehicleId = int.parse(preferences.getString('CurrentVehicleId') ?? "0");
    companyId = int.parse(preferences.getString('CurrentCompanyId'));
    depotId = int.parse(preferences.getString('CurrentDepotId'));
    questions = await getWellbeingCheckQuestions(companyId);
  ...
  _getUser() async {
    ...
    wellbeingNameController.text = name;
    wellbeingCheck.driverName = name;
  }

  initialiseWellbeingsCheck() {
    print("start wellbeing check");
    this.wellbeingCheck = new CreateWellbeingChecks(
        vehicleId,
        depotId,
        companyId,
        ...
        new List<CreateWellbeingCheckAnswers>(),
        location,
        location);
  ...
  CardSettingsWidget processQuestionType(String title, QuestionTypes type,
      CreateWellbeingCheckAnswers answerController) {
    switch (type) {
      ...
      case QuestionTypes.passFail:
        ...
        return new CardSettingsGenericWidget(children: [
          VimSwitch.CreateWellbeingSwitch(
      ...
      default:
        return VimSwitch.CreateWellbeingSwitch(
          controller: answerController,
          questionLabel: title,
          trueLabel: "Yes",
          falseLabel: "No",
  ...
  List<Widget> imageWidget(CreateWellbeingCheckAnswers answerController) {
    List<Widget> list = [];
    bool multiples =
  ...
  validate() {
    ...
    for (var i = 0; i < questions.length; i++) {
      var answer = wellbeingCheck.wellbeingChecksAnswers
    ...
    //If Wellbeing name is blank, display error message. At this point they would have been initialised so no need for null check.
    if (wellbeingCheck.driverName == "") {
      error = true;
    ...
    if (error) {
      ...
      sendWellbeingsChecks();
    }
  }

  sendWellbeingsChecks() async {
    print("sendWellbeingsChecks click");
    var token;
    var dio = new Dio();
    wellbeingCheck.currLocation = locationController.text;

    var dc = wellbeingCheck.toJson();
    ...
    try {
      int wellbeingCheckId;

      var response =
          await dio.post(BASE_URL + "/api/wellbeingchecks/Create", data: body);
      print(response.data.toString());
      wellbeingCheckId = response.data["wellbeingCheckId"];
      //wellbeingCheckId = 1;

      //Make a list of answers that have images, then Post Images
      List<CreateWellbeingCheckAnswers> imageAnswers = wellbeingCheck
          .wellbeingChecksAnswers
          .where((dca) =>
      ...
      //TODO SIGN
      CreateWellbeingCheckAnswers img =
          CreateWellbeingCheckAnswers(0, 0, "", "");
      ...
      await postWellbeingsChecksImages(
          context, wellbeingCheckId, imageAnswers, questions);
  ...
  List<Widget> getGroups() {
    List<Widget> list = [];
    List<CreateWellbeingCheckAnswers> answers =
        wellbeingCheck.wellbeingChecksAnswers;

    if (questions != null) {
      for (var i = 0; i < questions.length; i++) {
        //Instantiate and populate answers
        if (answers.length < questions.length) {
          CreateWellbeingCheckAnswers answer = new CreateWellbeingCheckAnswers(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            body: isLoading
                ? loadingSplash(
                    context, "Getting wellbeing check questions", "Please wait")
                : (isSending
                    ? loadingSplash(
                        context, "Sending wellbeing check", "Please wait")
                    : ListView(children: [
                        CardSettings(showMaterialonIOS: true, children: [
                          CardSettingsSection(children: [
                            CardSettingsGenericWidget(children: [
                              ...
                              //   contentOnNewLine: true,
                              //   label: "Driver Name",
                              //   controller: wellbeingNameController,
                              //   onChanged: (val) =>
                              //       {wellbeingCheck.driverName = val},
                              // ),
                              CardSettingsGenericWidget(children: [
                                Row(children: <Widget>[
                                  SizedBox(
