...
import 'package:vim_mobile/common/api_functions/lease_checks/getLeaseCheckQuestions.dart';
...
class StartLeaseCheckPage extends StatefulWidget {
  @override
  _StartLeaseCheckState createState() => new _StartLeaseCheckState();
}

class _StartLeaseCheckState extends State<StartLeaseCheckPage> {
  ...
  TextEditingController leaseNameController = TextEditingController();
  //TODO
  ...
  String backMessage =
      "Leaving this page will result in you losing all progress of your current lease check.";
  //TODO SIGN
  ...
  onImagePressed(CreateLeaseCheckAnswers answer) async {
  ...
  initialise() async {
    ...
    questions = await getLeaseCheckQuestions(companyId);
  ...
  CardSettingsWidget processQuestionType(String title, QuestionTypes type,
      CreateLeaseCheckAnswers answerController) {
    switch (type) {
      ...
      default:
        return VimSwitch.CreateLeaseSwitch(
          controller: answerController,
          questionLabel: title,
          trueLabel: "Yes",
          falseLabel: "No",
  ...
  validate() {
    ...
    for (var i = 0; i < questions.length; i++) {
      var answer = leaseCheck.leaseChecksAnswers
    ...
    //If Lease name is blank, display error message. At this point they would have been initialised so no need for null check.
    if (leaseCheck.driverName == "") {
      error = true;
  ...
  List<Widget> getGroups() {
    List<Widget> list = [];
    List<CreateLeaseCheckAnswers> answers = leaseCheck.leaseChecksAnswers;

    if (questions != null) {
      for (var i = 0; i < questions.length; i++) {
        //Instantiate and populate answers
        if (answers.length < questions.length) {
          CreateLeaseCheckAnswers answer = new CreateLeaseCheckAnswers(
                              ...
                              CardSettingsGenericWidget(children: [
                                ...
                                Padding(
                                  ...
                                  child: TextFormField(
                                    ...
                                    controller: leaseNameController,
                                    onChanged: (val) =>
                                        {leaseCheck.driverName = val},
                                  ),
                                )
                              ]),
                              CardSettingsGenericWidget(children: [
